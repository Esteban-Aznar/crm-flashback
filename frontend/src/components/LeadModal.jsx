...
const EMPTY = {
  ...
  precio_estimado: '', nombre_contacto: '', apellidos_contacto: '',
  email_contacto: '', telefono_contacto: '', provincia: '', comunidad_autonoma: '',
  ciudad: '', barrio: '', estado: 'contactado', notas: '',
  instrucciones_maquetacion: '', comercial_id: '',
};

export default function LeadModal({ lead, comerciales, onClose, onSave }) {
  const isEdit = !!lead;
  ...
  useEffect(() => {
    if (lead) {
      setForm({
        nombre_grupo: lead.nombre_grupo || '',
        tipo_grupo: lead.tipo_grupo || '',
        curso: lead.curso || '',
        pack: lead.pack || '',
        ...
        comunidad_autonoma: lead.comunidad_autonoma || '',
        ciudad: lead.ciudad || '',
        barrio: lead.barrio || '',
        estado: lead.estado || 'contactado',
        notas: lead.notas || '',
        instrucciones_maquetacion: lead.instrucciones_maquetacion || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSave({
        ...form,
        precio_estimado: form.precio_estimado === '' ? null : parseFloat(form.precio_estimado),
      ...
      setError(err.response?.data?.message || 'Error al guardar el lead.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={s.overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={s.modal}>
        <div style={s.header}>
          <h2 style={s.title}>{isEdit ? 'Editar Lead' : 'Nuevo Lead'}</h2>
        ...
        <form onSubmit={handleSubmit} style={s.body}>
          <Section label="Datos del grupo">
            <Field label="Nombre del grupo *" span={2}>
              <Input value={form.nombre_grupo} onChange={(v) => set('nombre_grupo', v)} placeholder="Ej: IES La Paz 2025" />
            </Field>
            ...
            <Field label="Precio estimado por alumno (€)">
              <Input type="number" min="0" step="0.01" value={form.precio_estimado} onChange={(v) => set('precio_estimado', v)} placeholder="0.00" />
            </Field>
          ...
          <Section label="Localización">
            ...
            <Field label="Comunidad autónoma">
              <Input value={form.comunidad_autonoma} onChange={() => {}} placeholder="Auto-completado" readOnly style={{ opacity: 0.6, cursor: 'default' }} />
            </Field>
            <Field label="Ciudad">
              <Input value={form.ciudad} onChange={(v) => set('ciudad', v)} placeholder="Ciudad" />
            </Field>
            <Field label="Barrio / Zona (opcional)">
              <Input value={form.barrio} onChange={(v) => set('barrio', v)} placeholder="Barrio o zona" />
            </Field>
          </Section>

          <Section label="Estado y notas">
            <Field label="Estado">
            ...
            <Field label="Notas (opcional)" span={2}>
              <Textarea value={form.notas} onChange={(v) => set('notas', v)} placeholder="Notas adicionales sobre el lead..." rows={3} />
            </Field>

            <Field label="Instrucciones de maquetación para el diseñador (opcional)" span={2}>
              <Textarea value={form.instrucciones_maquetacion} onChange={(v) => set('instrucciones_maquetacion', v)} placeholder="Instrucciones específicas para el diseño..." rows={3} />
            </Field>
          </Section>
...
function Input({ onChange, readOnly, style: extraStyle, ...props }) {
  return (
    <input
      {...props}
      readOnly={readOnly}
...
function Textarea({ value, onChange, rows = 3, placeholder }) {
  return (
    <textarea
      style={{ ...s.input, resize: 'vertical', minHeight: `${rows * 24 + 16}px` }}
      value={value}
...
const s = {
  ...
  label: { display: 'block', fontSize: '11px', fontWeight: 500, color: '#666', marginBottom: '6px', letterSpacing: '0.3px' },
  input: {
